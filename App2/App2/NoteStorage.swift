...
class NoteStorage{
    static let notes : NoteStorage = NoteStorage() //constant
    ...
    func setManagedContext(managedObjectContext: NSManagedObjectContext) {
        ...
        let notes = NotesDataHelper.readNotesFromCoreData(fromManagedObjectContext: self.managedObjectContext)
        currentIndex = NotesDataHelper.count
    ...
    func addNote(noteToBeAdded: Note) {
        if managedContextHasBeenSet {
            ...
            NotesDataHelper.createNoteInCoreData(
    ...
    func removeNote(at: Int) {
        if managedContextHasBeenSet {
            ...
            NotesDataHelper.deleteNoteFromCoreData(
    ...
    func readNote(at: Int) -> Note? {
        if managedContextHasBeenSet {
            ...
            noteReadFromCoreData = NotesDataHelper.readNoteFromCoreData(
    ...
    func changeNote(noteToBeChanged: Note) {
        if managedContextHasBeenSet {
            ...
            if noteToBeChangedIndex != nil {
                NotesDataHelper.changeNoteInCoreData(
